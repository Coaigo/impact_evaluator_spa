<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">TestDatas</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="testdata-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/testdatas/new"
          data-automation-id="testdata-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(testdatas ?? []) as unknown as TestData[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToTestData"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="testdata-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load testdatas: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * TestDatas List Page - Showcase of impact_spa_utils ease of use
 * 
 ...
 * from @coaigo/impact_spa_utils. Just provide your API call and you're done!
 */
import { computed } from 'vue'
import { api } from '@/api/client'
import { formatDate, ListPageSearch, useInfiniteScroll } from '@coaigo/impact_spa_utils'
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<TestData>({
  queryKey: ['testdatas'],
  queryFn: (params) => api.getTestDatas(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToTestData(_event: unknown, { item }: { item: TestData }) {
  router.push(`/testdatas/${item._id}`)
}
