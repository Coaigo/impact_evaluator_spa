<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">TestRuns</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="testrun-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/testruns/new"
          data-automation-id="testrun-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(testruns ?? []) as unknown as TestRun[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToTestRun"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="testrun-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load testruns: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * TestRuns List Page - Showcase of impact_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<TestRun>({
  queryKey: ['testruns'],
  queryFn: (params) => api.getTestRuns(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToTestRun(_event: unknown, { item }: { item: TestRun }) {
  router.push(`/testruns/${item._id}`)
}
